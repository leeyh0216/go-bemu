...
func TestOperationManifestRejectsUnknownDuplicateAndUnclassifiedInput(t *testing.T) {
	...
	tests := map[string]string{
		...
		"condition without evidence": strings.Replace(
			valid,
			"    conditions: []",
			"    conditions: [{setting: storage.read.enabled, equals: true, effect: service-availability, verification: transport, tests: [], en: Enabled., ko: 활성화합니다.}]",
			1,
		),
		"condition missing verification": strings.Replace(
			valid,
			"    conditions: []",
			"    conditions: [{setting: storage.read.enabled, equals: true, effect: service-availability, tests: [go:internal/transport/rest/example:TestOperation], en: Enabled., ko: 활성화합니다.}]",
			1,
		),
		"condition unknown verification": strings.Replace(
			valid,
			"    conditions: []",
			"    conditions: [{setting: storage.read.enabled, equals: true, effect: service-availability, verification: smoke, tests: [go:internal/transport/rest/example:TestOperation], en: Enabled., ko: 활성화합니다.}]",
			1,
		),
		"condition none verification": strings.Replace(
			valid,
			"    conditions: []",
			"    conditions: [{setting: storage.read.enabled, equals: true, effect: service-availability, verification: none, tests: [go:internal/transport/rest/example:TestOperation], en: Enabled., ko: 활성화합니다.}]",
			1,
		),
