...
func TestOperationManifestRejectsUnknownDuplicateAndUnclassifiedInput(t *testing.T) {
	...
	tests := map[string]string{
		...
		"condition without evidence": strings.Replace(
			valid,
			"    conditions: []",
			"    conditions: [{setting: storage.read.enabled, equals: true, effect: input-branch, verification: transport, tests: [], en: Enabled., ko: 활성화합니다.}]",
			1,
		),
		"condition missing verification": strings.Replace(
			valid,
			"    conditions: []",
			"    conditions: [{setting: storage.read.enabled, equals: true, effect: input-branch, tests: [go:internal/transport/rest/example:TestOperation], en: Enabled., ko: 활성화합니다.}]",
			1,
		),
		"condition unknown verification": strings.Replace(
			valid,
			"    conditions: []",
			"    conditions: [{setting: storage.read.enabled, equals: true, effect: input-branch, verification: smoke, tests: [go:internal/transport/rest/example:TestOperation], en: Enabled., ko: 활성화합니다.}]",
			1,
		),
		"condition none verification": strings.Replace(
			valid,
			"    conditions: []",
			"    conditions: [{setting: storage.read.enabled, equals: true, effect: input-branch, verification: none, tests: [go:internal/transport/rest/example:TestOperation], en: Enabled., ko: 활성화합니다.}]",
			1,
		),
