...
// It deliberately belongs to the REST catalog boundary, not the GoogleSQL
// parser boundary used for query and Storage Read expressions.
...
// validateDatasetMetadataView makes the IAM boundary explicit. BQEMU owns
// catalog metadata, but intentionally does not emulate dataset ACL/IAM data.
// An omitted view preserves the historical metadata response; callers that
// request a view must opt into the only complete local representation.
