...
func (w *Warehouse) createTable(ctx context.Context, table domain.Table) error {
	...
	if partitionType, ok := table.IngestionTimePartitioning(); ok {
		...
		columns = append(columns,
			partitionColumn+" TIMESTAMPTZ DEFAULT ("+ingestionTimePartitionDefault(partitionType)+") CHECK ("+
...
func ingestionTimePartitionBoundary(partitionType, expression string) string {
	return "date_trunc('" + strings.ToLower(partitionType) + "', " + expression + " AT TIME ZONE 'UTC') AT TIME ZONE 'UTC'"
}
