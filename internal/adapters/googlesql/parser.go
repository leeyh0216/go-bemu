...
var _ ports.QueryParameterValidator = (*Parser)(nil)
...
// ValidateQueryParameters enters the same upstream GoogleSQL AST parser used
// for the public syntax boundary. Lowering remains engine-owned, but malformed
// parameter syntax cannot reach job persistence or DuckDB.
func (*Parser) ValidateQueryParameters(ctx context.Context, request ports.QueryRequest) error {
	...
	if err != nil || output == nil {
		return invalid("invalid GoogleSQL query parameter syntax")
	}
	return nil
}

// ParseStorageReadPredicate parses a Storage Read row_restriction through the
// official GoogleSQL parser. The API accepts a predicate rather than a complete
// statement, so it is wrapped in a side-effect-free SELECT solely to obtain the
// typed WHERE expression. Engine adapters must lower the returned AST with
// bound values; they must not interpolate the input SQL.
func (*Parser) ParseStorageReadPredicate(ctx context.Context, predicate string) (gsql.ASTExpressionNode, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	...
	output, err := gsql.ParseStatement("SELECT 1 WHERE "+predicate, options)
	if err != nil || output == nil {
		return nil, invalid("invalid GoogleSQL row restriction syntax")
	}
	statement, err := output.Statement()
	if err != nil {
		return nil, parserFailure()
	}
	queryStatement, ok := statement.(*gsql.ASTQueryStatement)
	if !ok {
		return nil, parserFailure()
	}
	query, err := queryStatement.Query()
	if err != nil {
		return nil, parserFailure()
	}
	queryExpression, err := query.QueryExpr()
	if err != nil {
		return nil, parserFailure()
	}
	selectExpression, ok := queryExpression.(*gsql.ASTSelect)
	if !ok {
		return nil, parserFailure()
	}
	where, err := selectExpression.WhereClause()
	if err != nil || where == nil {
		return nil, parserFailure()
	}
	expression, err := where.Expression()
	if err != nil {
		return nil, parserFailure()
	}
...
func (*Parser) ParseDDL(ctx context.Context, request ports.QueryRequest) (domain.DDLCommand, bool, error) {
	...
	// The DDL parser is an optional catalog-mutation hook, not the general
	// query parser. Route ordinary DML straight to DuckDB: its dialect accepts
	// literals (such as DuckDB's TIMESTAMPTZ and struct literals) that are used
	// by the public tabledata contract but are not accepted by go-googlesql.
	upper := strings.ToUpper(request.SQL)
	if !strings.Contains(upper, "CREATE") && !strings.Contains(upper, "DROP") && !strings.Contains(upper, "ALTER") {
...
func parseCreateTable(node *gsql.ASTCreateTableStatement, request ports.QueryRequest) (domain.DDLCommand, error) {
	...
	return domain.DDLCommand{Kind: domain.DDLCreateTable, Table: reference, Schema: fields}, nil
}
...
func parseDropTable(node *gsql.ASTDropStatement, request ports.QueryRequest) (domain.DDLCommand, error) {
	...
	return domain.DDLCommand{Kind: domain.DDLDropTable, Table: reference}, nil
}
...
func parseAddColumn(action *gsql.ASTAddColumnAction, reference domain.TableReference) (domain.DDLCommand, error) {
	...
	return domain.DDLCommand{Kind: domain.DDLAddColumn, Table: reference, Field: field}, nil
}
...
func parseDropColumn(action *gsql.ASTDropColumnAction, reference domain.TableReference) (domain.DDLCommand, error) {
	...
	return domain.DDLCommand{Kind: domain.DDLDropColumn, Table: reference, Name: name}, nil
}
...
func parseRenameColumn(action *gsql.ASTRenameColumnAction, reference domain.TableReference) (domain.DDLCommand, error) {
	...
	return domain.DDLCommand{Kind: domain.DDLRenameColumn, Table: reference, Name: name, NewName: newName}, nil
}
...
func parseAlterColumnType(action *gsql.ASTAlterColumnTypeAction, reference domain.TableReference) (domain.DDLCommand, error) {
	...
	return domain.DDLCommand{Kind: domain.DDLAlterColumnType, Table: reference, Name: name, Field: field}, nil
}
