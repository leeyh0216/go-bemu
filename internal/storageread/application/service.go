...
func (s *Service) CreateSession(ctx context.Context, request domain.CreateSessionRequest) (domain.Session, error) {
	...
	record := domain.SessionRecord{
		...
		RowRestrictionBytes:  len(request.RowRestriction),
		Streams:              slices.Clone(streams),
		CreatedAt:            now, ExpireTime: session.ExpireTime,
	...
	state := &sessionState{
		session: session, snapshot: snapshot, retainedBytes: metadata.RetainedBytes, record: record,
	}
