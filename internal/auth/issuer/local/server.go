...
func NewServer(manifestPath string, logger *slog.Logger) (*Server, error) {
	...
	server := &Server{manifest: manifest, logger: logger}
	...
	if server.account.TokenURI != manifest.OAuthTokenURL || server.user.TokenURI != manifest.OAuthTokenURL ||
		server.external.TokenURL != manifest.STSTokenURL || server.external.CredentialSource["file"] != manifest.SubjectToken {
...
func (s *Server) oauthToken(writer http.ResponseWriter, request *http.Request) {
	...
	case issuerdomain.GrantTypeRefreshToken:
		if !allowedFields(form, "grant_type", "client_id", "client_secret", "refresh_token", "scope") ||
			...
			writeOAuthError(writer, http.StatusBadRequest, issuerdomain.ErrorInvalidGrant)
			return
		}
		s.writeToken(writer, false, form.Get("scope"))
	case issuerdomain.GrantTypeJWTBearer:
	...
	default:
		writeOAuthError(writer, http.StatusBadRequest, issuerdomain.ErrorUnsupportedGrantType)
	}
}
...
func (s *Server) stsToken(writer http.ResponseWriter, request *http.Request) {
	...
	if !allowedFields(form, "grant_type", "audience", "resource", "scope", "requested_token_type", "subject_token", "subject_token_type", "actor_token", "actor_token_type", "options") ||
		form.Get("grant_type") != issuerdomain.GrantTypeTokenExchange ||
		form.Get("requested_token_type") != issuerdomain.TokenTypeAccessToken ||
		...
		writeOAuthError(writer, http.StatusBadRequest, issuerdomain.ErrorInvalidRequest)
		return
	}
...
func (s *Server) writeToken(writer http.ResponseWriter, sts bool, scope string) {
	...
	if _, err := rand.Read(tokenBytes); err != nil {
		writeOAuthError(writer, http.StatusInternalServerError, issuerdomain.ErrorServer)
		return
	}
	response := issuerdomain.TokenResponse{
		AccessToken: base64.RawURLEncoding.EncodeToString(tokenBytes),
		TokenType:   issuerdomain.BearerTokenType, ExpiresIn: 3600, Scope: scope,
	}
	if sts {
		response.IssuedTokenType = issuerdomain.TokenTypeAccessToken
	}
	writer.Header().Set("Content-Type", "application/json")
...
func (s *Server) verifyAssertion(assertion string, now time.Time) error {
	...
	if decodeJWTPart(parts[0], &header) != nil || decodeJWTPart(parts[1], &claims) != nil ||
		...
		claims.Issuer != s.account.ClientEmail || claims.Audience != s.manifest.OAuthTokenURL ||
...
func writeOAuthError(writer http.ResponseWriter, status int, code issuerdomain.OAuthErrorCode) {
	writer.Header().Set("Content-Type", "application/json")
	writer.Header().Set("Cache-Control", "no-store")
