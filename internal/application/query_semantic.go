...
func (s *QueryService) prepareQueryAdmission(
	...
	if matcher, ok := s.operationAnalyzer.(ports.AnalyzedQueryOperationAnalyzer); ok {
		operation, matched, err := matcher.AnalyzeStatementOperation(ctx, statement, request)
		if err != nil {
			return preparedQuery{}, err
		}
		if matched {
			return preparedQuery{
				...
				valid: true, operation: operation, operationMatched: true,
			}, nil
		}
	}
