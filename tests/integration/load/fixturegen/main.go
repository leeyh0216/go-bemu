...
func validateManifest(value manifest, observe bool) error {
	...
	for _, object := range value.Objects {
		...
		if cleaned != object.Name || strings.HasPrefix(cleaned, "/") || strings.HasPrefix(cleaned, "../") {
			return errors.New("fixture shape is invalid")
		}
		switch object.Kind {
		case "parquet":
			if object.Query == "" || object.Content != "" || object.Rows < 1 {
				return errors.New("Parquet fixture shape is invalid")
			}
		case "corrupt":
			if object.Query != "" || object.Content == "" || len(object.Content) > 4096 || object.Rows != 0 {
				return errors.New("corrupt fixture shape is invalid")
			}
		default:
			return errors.New("fixture kind is invalid")
		}
...
func generate(ctx context.Context, outputRoot string, locked manifest, observe bool) error {
	...
	for _, object := range locked.Objects {
		...
		if object.Kind == "corrupt" {
			if err := os.WriteFile(path, []byte(object.Content), 0o600); err != nil {
				return err
			}
		} else {
