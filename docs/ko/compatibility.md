...
| 인증 비활성화 | 구현되어 있습니다. 인증 정보를 무시하고 익명 주체를 사용합니다. |
| `bearer-present` | 형식과 존재 여부만 확인합니다. 신원을 증명하지 않습니다. |
| 크기에 상한을 둔 정적 토큰 집합 | 구현되어 있습니다. 시작 시 오류가 나면 실행하지 않고, 다시 읽기에 실패하면 모두 거부했다가 유효한 파일로 복구합니다. |
| ADC/OAuth/STS/WIF 토큰 획득 | 클라이언트가 담당합니다. 최종 Bearer 토큰에는 설정한 로컬 정책을 적용할 수 있습니다. |
| IAM 권한 확인 | 지원하지 않습니다. |
...
인증](https://cloud.google.com/docs/authentication)에 따라 구분합니다. 공개 API 경계는
[RFC 6750](https://www.rfc-editor.org/rfc/rfc6750#section-2.1)에 따라 `Authorization`
헤더와 메타데이터를 해석합니다. 로컬 정적 토큰 확인과 클라이언트가 수행하는 토큰
획득을 IAM과 같은 기능으로 설명하면 안 됩니다.
