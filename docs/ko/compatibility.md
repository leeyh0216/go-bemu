...
기본 스트림 CDC는 부분 지원입니다. 선언한 기본 키, 쌍을 이룬 ProtoRows 의사 열,
순서가 있는 UPSERT/DELETE, 키별로 유지되는 로컬 원장을 구현했습니다. 비동기 적용,
staleness, 릴리스 client 복구·병렬 writer E2E는 아직 지원하지 않습니다. 누락된 값의 기본
표현식, 한쪽 저장소만 복원한 상태의 증명, 분산 쓰기 동시성은 지원하지 않습니다. SQLite는 스트림, 확인 기록, 커밋 그룹의 단계를 영속화하며 시작할
