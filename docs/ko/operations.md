...
디렉터리 경로, 종료 제한 시간, 로그, 관리 API, UI, 두 Storage 서비스, 로드
기능의 설정도 이곳에서 사용합니다.
...
`load.gcsEndpoint`에는 fake-GCS 호환 JSON 서비스의 절대 HTTP(S) URL을 항상
지정해야 합니다. 로드 원본은 `gs://` 객체 URI만 허용하며, 객체 목록 조회와
다운로드에는 설정된 제한을 적용합니다.
