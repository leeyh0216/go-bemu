...
임시 작업 디렉터리에 내려받습니다. 복수 URI와 와일드카드 결과는 URI 순서로 정렬하고
중복 제거합니다. 불변 계획은 숫자 GCS generation, 선택적 ETag, 바이트 크기를 결합합니다.
겹친 패턴의 메타데이터가 다르거나 다운로드 generation이 바뀌면 loader 실행 전에
실패합니다. 안정적인 mutation ID는 작업 identity와 이 계획을 결합합니다. DuckDB는
출력 영수증을 대상 트랜잭션에 함께 기록하므로 엔진 재시작 뒤 동일한 요청은 읽기
전용으로 처리하고, 같은 ID에 다른 계획을 결합하면 거부합니다.
...
반영하고, 기준 SQLite 메타데이터는 커밋 뒤 게시합니다. 새 대상의 시간 단위 또는 정수
범위 파티션 메타데이터, 파티션 만료 시간, 순서가 있는 클러스터 필드도 같은 불변 적재
계획에 포함합니다. 수집 시간 파티션 대상은 `_PARTITIONTIME`과 `_PARTITIONDATE`를
생성하고 채웁니다. 적재 요청으로 기존 대상의 구성을 바꿀 수는 없습니다. 파티션
데코레이터는 객체에 접근하기 전에 기준 파티션 구성과 대조합니다. 시간 컬럼과 정수
범위 파티션의 입력 행은 지정 경계 안에 있어야 하며, 수집 시간 적재는 요청한 경계를
