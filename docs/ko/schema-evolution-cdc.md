...
현재 Parquet 적재는 기존 테이블을 기준으로 유형 변환을 검증합니다. 쓰기
...
기준 시험 데이터에서는 민감한 값을 제거해야 합니다. 성공하는 구조와 실패하는
구조를 모두 포함해야 합니다. E2E는 출시된 클라이언트로 공개 REST/gRPC API를
...
fingerprint=<민감한 값을 제거한 결정적 요약 해시>
fix_hint=<다음 조치 경계>
```

`fingerprint`는 기준 스키마나 민감한 값을 제거한 데이터 구조에서 계산합니다. 인증
정보나 운영 환경의 행 값은 포함하지 않습니다.
