...
없는 정상 종료 절차는 호환성 오류를 재현하기 어렵게 합니다. BigQuery 호환 REST와 Storage RPC 리스너는 외부에 공개하는
...
3. 시작, 요청, 최종 상태 확인, 종료 테스트에는 이름이 있고 설정으로 조정할 수
   있는 제한 시간을 사용합니다. 원본 진단 정보는 크기를 제한해 보관합니다.
4. 릴리스 컨테이너는 루트 권한이 아닌 사용자로 실행합니다. 운영 프로필에는 읽기
   ...
   하나의 정상 종료 기한을 사용합니다.
5. 진단 기능은 요청, 응답, 메타데이터, SQL, 행, Protobuf, 백엔드 오류의 원본 맥락을
   보존합니다. 크기 제한으로 자원 사용을 통제하고, 로그의 접근과 보관은 배포 환경이
   관리합니다. 전송 데이터 기록을 켜고 끄는 설정은 두지 않습니다.
