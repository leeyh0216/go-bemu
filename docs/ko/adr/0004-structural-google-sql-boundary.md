...
의미 분석 어댑터가 마련되기 전에는 일반 GoogleSQL을 지원하지 않습니다. 커넥터
템플릿 규칙을 추가할 때는 정확한 버전, 지문, 공식 근거, 실패 사례, 제거 조건을
기록합니다. SQL DDL은 BigQuery 기준 메타데이터를 함께 반영해야 합니다.
물리 카탈로그만 변경해서는 안 됩니다.
