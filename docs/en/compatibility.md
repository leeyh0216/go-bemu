...
| table insert/get/delete | Verified basic | standard table and canonical schema metadata; `tables.get` supports `BASIC`, top-level `selectedFields`, and deterministic `STORAGE_STATS`; `FULL` and IAM metadata remain unsupported |
| table list | Verified basic | paging; table-list views and IAM metadata remain unsupported |
...
| semantic SQL DDL | Partial | The official GoogleSQL AST/analyzer executes `CREATE [OR REPLACE] VIEW`, `DROP VIEW`, `CREATE TABLE`, `DROP TABLE`, `TRUNCATE TABLE`, top-level `ADD`/`RENAME`/`DROP COLUMN`, and `ALTER COLUMN SET DATA TYPE`; view dependency cycles and dependent schema changes fail before physical mutation, while crash recovery between SQLite and the engine remains #26 |
...
| physical load mutation receipt | The job identity and immutable plan produce a stable mutation ID. DuckDB stores the result receipt in the same transaction as destination rows, so an identical retry after engine restart returns the prior result rather than applying the load twice. Reusing the ID with a different plan fails before staging. SQLite startup publication reconciliation remains tracked by #26. |
...
| load destination partitioning and clustering | New destinations preserve time-unit or integer-range partition metadata, partition expiration, and one-to-four valid clustering fields. Ingestion-time destinations populate `_PARTITIONTIME` and `_PARTITIONDATE`; existing destinations accept only an identical requested layout. `$yyyy`, `$yyyymm`, `$yyyymmdd`, `$yyyymmddhh`, and integer-range-start decorators address an existing partition. Input rows must match column/range decorators, while ingestion-time decorators assign the requested boundary. Decorated `WRITE_EMPTY` and `WRITE_TRUNCATE` inspect or replace only that partition. Partition expiration sweeping and physical DuckDB reclustering remain tracked by #47. |
